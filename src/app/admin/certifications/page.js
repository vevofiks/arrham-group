...
const CertificationsPage = () => {
  ...
  const openModal = (type, cert = null) => {
    setModalType(type);
  ...
  const openDeleteConfirm = (certId) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/certifications/${deleteTarget}/`, {
        method: "DELETE",
      });
      if (res.ok) {
        fetchCerts();
